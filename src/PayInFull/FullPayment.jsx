...
//     navigate(`/table/${tableNumber}/ThankYou`);
...
//   // If caller passed totalAmount in navigation state, use it as initial value.
//   const initialTotal =
//     (location && location.state && Number(location.state.totalAmount)) || 0;
...
//   // If no totalAmount was provided, fetch it from backend by tableNumber
//   useEffect(() => {
//     if (totalAmount > 0) return; // already have value
...
//     fetch(`/api/tables/${tableNumber}/bill`)
//       .then((res) => {
...
//       .then((data) => {
//         if (!mounted) return;
//         // Expect backend to return { totalAmount: 123.45 } or similar
//         const amt = Number(data?.totalAmount) || 0;
//         setTotalAmount(amt);
//       })
//       .catch((err) => {
//         if (!mounted) return;
//         setFetchError(err.message || "Failed to load bill");
//       })
//       .finally(() => {
//         if (!mounted) return;
//         setLoading(false);
...
//       // After successful payment, navigate to Receipt page.
//       // Pass payment details and totalAmount via location.state so Receipt can show them.
//       navigate(`/table/${tableNumber}/receipt`, {
//         state: { paymentInfo, totalAmount },
//       });
//     } catch (err) {
//       console.error("Payment failed:", err);
//       // You may set UI error state here to show message to user
//     } finally {
//       setProcessing(false);
//     }
//   };
...
//             </div>
//             {fetchError && (
//               <p className="error">Could not load bill: {fetchError}</p>
//             )}
//             <p className="amount-description">
//               Complete bill for Table #{tableNumber}
//             </p>
...
//               disabled={loading || processing || totalAmount <= 0}
//             >
...
//       setError(null);
//       try {
//        console.log('Fetching menu items from:', `http://localhost:3000/menu/${tableNumber}`);
...
//         if (!response.ok) {
//           console.log('Table-specific endpoint failed, trying general menu endpoint');
...
//         setMenuItems(items);
//         console.log(':white_check_mark: Loaded menu items for full payment:', items.length, 'items');
//         console.log('First few items:', items.slice(0, 3));
//       } catch (err) {
//         console.error(':x: Error loading menu:', err);
...
//     return selectedItems.reduce((total, item) => total + (parseFloat(item.prices) || 0), 0);
//   };
...
//                 <p>:arrows_counterclockwise: Loading menu items for Table {tableNumber}...</p>
...
//                 </p>
//                 <button onClick={() => window.location.reload()}>:arrows_counterclockwise: Retry</button>
//               </div>
//             )}
//             {!loading && !error && menuItems.length === 0 && (
//               <div className="error-message">
//                 <p>:warning: No menu items found for Table {tableNumber}</p>
//                 <p style={{ fontSize: '0.9rem', color: '#666' }}>
...
//                 <div style={{ padding: '10px', backgroundColor: '#F0F8FF', marginBottom: '10px', borderRadius: '5px' }}>
//                   <small>:white_check_mark: Loaded {menuItems.length} menu items from backend</small>
//                 </div>
//                 {menuItems.map((item) => (
//                   <div
//                     key={item.id}
...
//             <button
//               type="submit"
//               className={`payment-submit-button ${selectedItems.length === 0 ? 'disabled' : ''}`}
