    ...
    const loadMenu = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const generalResponse = await fetch('http://localhost:3000/menu');
  ...
  return (
    <div className="split-evenly-payment-container">
      ...
      <header className="payment-header">
        ...
        <p className="table-info">Table #{tableNumber} • {numberOfGuests} Guests</p>
      </header>
        ...
        <div className="payment-section">
          ...
          {currentStep === 'menu-selection' && (
            <>
              ...
              {selectedItems.length > 0 && (
                <div className="selection-summary">
                  ...
                  <div className="split-preview">
                    Split evenly among {numberOfGuests} guests: <strong>${splitAmount.toFixed(2)} each</strong>
                  </div>
                </div>
              )}
              ...
              {/* Continue Button */}
              <button 
                className={`continue-button ${selectedItems.length === 0 ? 'disabled' : ''}`}
                onClick={handleMenuSelectionComplete}
                disabled={selectedItems.length === 0}
              >
                {selectedItems.length === 0 
                  ? 'Select Items to Continue' 
                  : `Continue to Payment - ${numberOfGuests} Guest${numberOfGuests > 1 ? 's' : ''}`
                }
              </button>
            </>
          )}
          ...
          {currentStep === 'payment-details' && (
            <>
              <h2>Split Evenly Payment - Guest {currentGuestIndex + 1} of {numberOfGuests}</h2>
              ...
              <div className="bill-display">
                ...
                <div className="breakdown-item">
                  ...
                  <span className="breakdown-value">${calculateTotal().toFixed(2)}</span>
                </div>
                <div className="breakdown-item">
                ...
                <div className="breakdown-calculation">
                  <span className="calculation-text">
                    ${calculateTotal().toFixed(2)} ÷ {numberOfGuests} = ${splitAmount.toFixed(2)}
                  </span>
                </div>
                  ...
                  {guestPayments.map((guest, index) => (
                    <div 
                      ...
                      className={`guest-indicator ${
                        index === currentGuestIndex ? 'current' : 
                        guest.isPaid ? 'completed' : 'pending'
                      }`}
                    >
                      Guest {guest.guestNumber}
                      {guest.isPaid && <span className="checkmark">✓</span>}
                      {index === currentGuestIndex && <span className="current-arrow">→</span>}
                    </div>
                  ))}
                </div>
              </div>
